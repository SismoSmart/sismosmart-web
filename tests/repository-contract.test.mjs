...
test("DNS cutover audit protects delegation, origin services, and legacy isolation", () => {
  ...
  assert.match(config.originIpv4, /^(?:\d{1,3}\.){3}\d{1,3}$/);
  assert.match(config.legacyIpv4, /^(?:\d{1,3}\.){3}\d{1,3}$/);
  assert.notEqual(config.originIpv4, config.legacyIpv4);
...
test("browser runner is portable, animation-stable, and retry-safe", () => {
  ...
  assert.match(runner, /MAX_APP_START_ATTEMPTS/);
  assert.match(runner, /isAddressInUseFailure/);
